...
- Save Changes applies visible panel edits before writing JSON.
...
Use the top-bar **Settings** button or press `Ctrl+,` to edit the wallboard visually. The settings window can be resized or maximized, and can update the wallboard title, default layout, rotation options, and the panel list. **Save Changes** applies the currently visible panel editor values, writes `wallboard.json`, verifies the saved file, creates a `wallboard.backup.json` copy of the previous file, and reloads the wallboard.
